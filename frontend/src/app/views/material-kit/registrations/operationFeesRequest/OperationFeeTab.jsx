...
export default function OperationFeeTab(props) {
  console.log("🔥 تمام Props دریافتی OperationFeeTab:", props);
  ...
  useEffect(() => {
    console.log("🔍 ====== بارگذاری اولیه OperationFeeTab ======");
    fetchAllRequests();
  }, []);

  const fetchAllRequests = async () => {
    console.log("📡 دریافت تمام درخواست‌های عملیات...");
    setLoading(true);
    setFetchError(null);
    
    try {
      const response = await api.get('/operation-requests/all');
      ...
      if (response.data?.success && response.data?.data) {
        ...
        toast.success(`✅ ${data.total_requests || 0} درخواست عملیات دریافت شد`);
      } else {
  ...
  const fetchFeeRecords = async () => {
    try {
      console.log("💳 دریافت فیس‌های ثبت شده عملیات...");
      const response = await api.get('/operation-fees');
  ...
  const handleOpenFeeForm = (request) => {
    ...
    setFeeFormData({
      ...
      discount: "",
      payment_method: "cash",
      description: `عملیات: ${request.operation_type || ''} - ${request.operation_name || ''}`,
  ...
  const handleSubmitFee = async (e) => {
    ...
    try {
      const payload = {
        ...
        operation_request_ids: selectedRequest ? [selectedRequest.id] : []
      };
      ...
      if (editingFee) {
        ...
        response = await api.post(`/operation-fees/registration/${selectedRequest.reg_id}`, payload);
    ...
    <div>
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px' }}>
          <div>
            ...
            <div style={{ color: '#9ca3af', fontSize: '13px', marginTop: '5px' }}>
              تمام درخواست‌های عملیات تمام مراجعه‌کنندگان
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
            <button
              ...
              style={{
                backgroundColor: filterMode === 'all' ? '#dc2626' : '#374151',
                color: 'white',
                padding: '6px 15px',
                borderRadius: '6px',
                border: 'none',
        ...
        <div style={{
          ...
          <div>
            <span style={{ color: '#dc2626', fontSize: '11px' }}>👥 مراجعه‌کنندگان</span>
            <div style={{ color: '#dc2626', fontWeight: 'bold' }}>{groupedRequests.length}</div>
          </div>
        </div>
      ...
      {loading || loadingRequests ? (
        ...
        <div style={{
          ...
          <div>
            ...
            {filterMode === 'all' && 'هیچ درخواست عملیات ثبت نشده است'}
          </div>
                ...
                <div style={{ flex: 1, minWidth: '250px' }}>
                  <div style={{ display: 'flex', gap: '15px', flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    <span style={{ color: '#dc2626', fontWeight: 'bold' }}>
                      {request.operation_type || 'عملیات'}
                    </span>
                    {request.operation_name && (
                      <span style={{ color: 'white' }}>{request.operation_name}</span>
                    )}
                    {request.barcode && (
                      <span style={{ 
                        color: '#fcd34d', 
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  <div style={{ 
                    ...
                    {selectedRequest.operation_type || 'عملیات'}
                    {selectedRequest.operation_name && ` - ${selectedRequest.operation_name}`}
