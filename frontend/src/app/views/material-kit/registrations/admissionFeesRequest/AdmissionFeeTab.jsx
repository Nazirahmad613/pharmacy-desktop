// src/app/pages/admissionFeesRequest/AdmissionFeeTab.jsx

import { useState, useEffect } from "react";
import { toast } from "react-toastify";

export default function AdmissionFeeTab(props) {
  console.log("🔥 تمام Props دریافتی AdmissionFeeTab:", props);
  ...
  // ============ بارگذاری اولیه ============
  useEffect(() => {
    console.log("🔍 ====== بارگذاری اولیه AdmissionFeeTab ======");
    fetchAllRequests();
  }, []);

  // ============ دریافت تمام درخواست‌های بستری ============
  const fetchAllRequests = async () => {
    console.log("📡 دریافت تمام درخواست‌های بستری...");
    setLoading(true);
    setFetchError(null);
    ...
    try {
      ...
      if (response.data?.success && response.data?.data) {
        ...
        toast.success(`✅ ${data.total_requests || 0} درخواست بستری دریافت شد`);
      } else {
  ...
  // ============ دریافت فیس‌های ثبت شده بستری ============
  const fetchFeeRecords = async () => {
    try {
      console.log("💳 دریافت فیس‌های ثبت شده بستری...");
      const response = await api.get('/admission-fees');
  ...
  const handleOpenFeeForm = (request) => {
    ...
    setFeeFormData({
      ...
      discount: "",
      payment_method: "cash",
      description: `بستری: ${request.admission_type || ''} - ${request.ward_name || ''}`,
  ...
  const handleSubmitFee = async (e) => {
    ...
    try {
      const payload = {
        ...
        admission_request_ids: selectedRequest ? [selectedRequest.id] : []
      };
      ...
      let response;
      if (editingFee) {
        response = await api.put(`/admission-fees/${editingFee.id}`, payload);
        toast.success("✅ فیس بستری با موفقیت ویرایش شد");
      } else {
        response = await api.post(`/admission-fees/registration/${selectedRequest.reg_id}`, payload);
        toast.success("✅ فیس بستری با موفقیت ثبت شد");
      }
  ...
  const getFeeForRequest = (requestId) => {
    ...
    return feeRecords.find(fee => 
      fee.admission_request_ids && fee.admission_request_ids.includes(requestId)
    );
  };
    ...
    <div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px' }}>
          <div>
            ...
            <div style={{ color: '#9ca3af', fontSize: '13px', marginTop: '5px' }}>
              تمام درخواست‌های بستری تمام مراجعه‌کنندگان
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
            <button
              ...
              style={{
                backgroundColor: filterMode === 'all' ? '#ef4444' : '#374151',
                color: 'white',
                padding: '6px 15px',
                borderRadius: '6px',
                border: 'none',
        ...
        <div style={{
          ...
          <div>
            <span style={{ color: '#ef4444', fontSize: '11px' }}>👥 مراجعه‌کنندگان</span>
            <div style={{ color: '#ef4444', fontWeight: 'bold' }}>{groupedRequests.length}</div>
          </div>
        </div>
                ...
                <div style={{ flex: 1, minWidth: '250px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '15px', flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    <span style={{ color: '#ef4444', fontWeight: 'bold' }}>
                      {request.admission_type || 'بستری'}
                    </span>
                    {request.ward_name && (
                      <span style={{ color: 'white' }}>بخش: {request.ward_name}</span>
                    )}
                    {request.barcode && (
                      <span style={{ 
                        color: '#fcd34d', 
      ...
      {(showFeeForm && (selectedRequest || editingFee)) && (
        <div style={{
          ...
          <div style={{
            ...
            <h4 style={{ color: '#ef4444', marginBottom: '20px' }}>
              {editingFee ? '✏️ تصحیح فیس بستری' : '💰 اخذ فیس بستری'}
            </h4>
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  <div style={{ 
                    ...
                    {selectedRequest.admission_type || 'بستری'}
                    {selectedRequest.ward_name && ` - بخش: ${selectedRequest.ward_name}`}
