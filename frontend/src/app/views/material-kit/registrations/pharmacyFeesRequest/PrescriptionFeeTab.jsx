...
export default function PrescriptionFeeTab({ api }) {
  ...
  const fetchFees = async (page = 1) => {
    setLoading(true);
    try {
      const response = await api.get(`/prescription-fees?page=${page}&per_page=${ROWS_PER_PAGE}`);
      ...
      toast.error("❌ خطا در دریافت لیست فیس‌های نسخه");
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchStatistics = async () => {
    try {
      const response = await api.get("/prescription-fees/statistics");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await api.put(`/prescription-fees/${editingId}`, formData);
        toast.success("✅ فیس نسخه با موفقیت ویرایش شد");
      } else {
        await api.post("/prescription-fees", formData);
        toast.success("✅ فیس نسخه با موفقیت ثبت شد");
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await api.delete(`/prescription-fees/${id}`);
      toast.success("✅ فیس نسخه با موفقیت حذف شد");
      fetchFees(currentPage);
      fetchStatistics();
    } catch (err) {
      toast.error("❌ خطا در حذف فیس نسخه");
    }
  };
  ...
  return (
    <div>
      ...
      >
        {showForm ? '✕ بستن فرم' : '+ ثبت فیس نسخه جدید'}
      </button>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} style={{
          ...
          <h4 style={{ color: '#60a5fa', marginBottom: '15px' }}>
            {editingId ? '✏️ ویرایش فیس نسخه' : '📝 ثبت فیس نسخه'}
          </h4>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan="10" style={{ textAlign: 'center', padding: '30px', color: '#9ca3af' }}>
                  <div>📭 هیچ فیس نسخه‌ای ثبت نشده است</div>
                </td>
              </tr>
            )}
          </tbody>
