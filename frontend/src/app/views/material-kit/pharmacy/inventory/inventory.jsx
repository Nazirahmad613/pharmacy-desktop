...
const Stock = () => {
    ...
    const getSortIcon = (field) => {
        if (sortField !== field) {
            return (
                <FaSortAmountDown
                    className="inline mr-1 opacity-30"
                    size={12}
                />
            );
        }
    ...
    const filteredStocks = stocks.filter((stock) => {
        ...
        const medicationName = getMedicationName(stock).toLowerCase();

        const supplierName = String(stock.supplier_name || '').toLowerCase();

        const barcode = String(getBarcode(stock) || '').toLowerCase();

        const batchNumber = String(stock.batch_number || '').toLowerCase();
        ...
        const matchesSupplier =
            !filterSupplier ||
            stock.supplier_name === filterSupplier;
    ...
    const uniqueSuppliers = [
        ...new Set(stocks.map((s) => s.supplier_name).filter(Boolean))
    ];


    const uniqueTypes = [
        ...new Set(stocks.map((s) => s.type).filter(Boolean))
    ];
        ...
        return (
            ...
            >
                <div className="flex items-center justify-between">
                    ...
                    <div className="text-3xl opacity-70" style={{ color: color }}>
                        {icon}
                    </div>
                </div>
            </div>
                ...
                <div className="flex justify-center items-center h-96 bg-white">
                    <div className="text-center bg-white rounded-xl shadow-lg p-8 max-w-md">
                        ...
                        <p className="text-gray-600 mb-4">
                            {errorMessage ||
                                'شما دسترسی به این صفحه را ندارید. لطفاً دوباره وارد شوید.'}
                        </p>
                ...
                <div className="flex justify-between items-center mb-6">
                    <div>
                        ...
                        <p className="text-gray-500 text-sm">
                            مدیریت و نظارت بر موجودی داروها و اقلام انبار
                        </p>
                    </div>
                                        ...
                                        >
                                            ...
                                            <td className="px-4 py-3 whitespace-nowrap">
                                                <span className="text-sm font-bold text-green-700">
                                                    ...
                                                    stock.selling_price !== ''
                                                        ? Number(stock.selling_price).toLocaleString()
                                                        : '---'}
                                                </span>
                                            </td>
                                            ...
                                            <td className="px-4 py-3 whitespace-nowrap">
                                                <div className="flex items-center gap-1">
                                                    ...
                                                    {(stock.status_color === 'yellow' ||
                                                        stock.status_color === 'orange') && (
                                                        <FaClock className="text-orange-600 text-sm" />
                                                    )}

                                                    {stock.status_color === 'red' && (
