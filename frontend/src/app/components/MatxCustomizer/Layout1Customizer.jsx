...
export default function Layout1Customizer({ settings, handleChange, handleControlChange }) {
  ...
  return (
    <Fragment>
      <Box mb="16px" mx="12px">
        <ThemeName>{t("Sidebar theme")}</ThemeName>
        <ToolbarContainer>
      ...
      <Box mb="18px" mx="12px">
        <FormControl component="fieldset">
          <FormLabel component="legend">{t("Sidebar mode")}</FormLabel>
          <RadioGroup
            aria-label="Sidebar"
            name="leftSidebar"
            value={settings.layout1Settings.leftSidebar.mode}
            onChange={handleControlChange("layout1Settings.leftSidebar.mode")}>
            <FormControlLabel value="full" control={<Radio />} label={t("Full")} />
            <FormControlLabel value="close" control={<Radio />} label={t("Close")} />
            <FormControlLabel value="compact" control={<Radio />} label={t("Compact")} />
          </RadioGroup>
        </FormControl>
      </Box>

      <Box mb="32px" mx="12px">
        <ThemeName>{t("Sidebar background image")}</ThemeName>
        <div>
      ...
      <Box mb="24px" mx="12px">
        <FormControl component="fieldset">
          <FormLabel component="legend">{t("Topbar")}</FormLabel>
          <FormGroup>
            <FormControlLabel
              control={
                <Switch
