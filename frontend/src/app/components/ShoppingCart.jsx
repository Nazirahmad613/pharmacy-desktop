...
export default function ShoppingCart({ container }) {
  const { t } = useTranslation(); // اضافه کردن قابلیت ترجمه
        ...
        <Drawer
          ...
          <MiniCart>
            ...
            <Box flexGrow={1} overflow="auto">
              {cartList.map((product, i) => (
                <ProductBox key={i}>
                  ...
                  <ProductDetails>
                    <H6>{t(`cart.products.${product.id}.title`, { defaultValue: product.title })}</H6>
                    <Small color="text.secondary">
                      {t("cart.price")} ${product.price} x {product.qty}
                    </Small>
                  </ProductDetails>
            ...
            <Button
              ...
              {t("cart.checkout")} (${totalCost.toFixed(2)})
            </Button>
          </MiniCart>
        </Drawer>
      </ThemeProvider>
