...
export default function SecondarySidebarToggle() {
  ...
  const { t } = useTranslation(); // مقدار ترجمه
  ...
  return (
    <Toggle className={clsx({ open: settings.secondarySidebar.open })}>
      {settings.secondarySidebar.open && (
        <IconButton onClick={toggle} size="small" aria-label={t("sidebar.close")}> {/* ترجمه */}
          <Close sx={{ color: "primary.contrastText" }} />
        </IconButton>
      )}

      {!settings.secondarySidebar.open && (
        <Fab color="primary" aria-label={t("sidebar.open")} onClick={toggle}> {/* ترجمه */}
