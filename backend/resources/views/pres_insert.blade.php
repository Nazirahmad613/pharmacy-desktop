...
<body> 
    ...
    <div class="form-container">
        ...
        <form action="{{ route('pres_store') }}" method="POST">
            @csrf

         <label for="pa_name">اسم مریض</label>
        <input id="pa_name" type="text" name="pa_name" required>

         <label for="pa_age">سن مریض</label>
        <input id="pa_age" type="number" name="pa_age" required>
  ...
  <select name="doc_id" id="doc_id" required>
   ...
   <h3>انتخاب  اسم دوا </h3>
  <select name="med_id" id="med_id" required>
    <option value="">انتخاب کنید</option>
    @foreach($medicate as $medications)
        <option value="{{ $medications->med_id }}">{{ $medications->gen_name }}</option>
    @endforeach
</select>
           ...
           <input id="quantity" type="number" name="quantity" required>
            ...
            <label for="pres_date">تاریخ احرآ نسخه </label>
            <input id="pres_date" type="date" name="pres_date" required>
